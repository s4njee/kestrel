//! main.rs — Desktop binary entry point for the sftpapp Tauri shell.
//!
//! This is a thin launcher: all application wiring lives in the library crate
//! (`sftpapp_lib`, see `lib.rs`) so the same setup code can be reused by the
...
///
/// Arguments: none.
/// Returns: `()` — delegates to [`sftpapp_lib::run`], which blocks until the
