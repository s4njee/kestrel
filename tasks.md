...
- [x] **E8-S7 — Remote search** (M) Depends: E8-S1
...
- [x] **E8-S9 — Multiple concurrent sessions (host tabs)** (L)
...
- **E8-S9 — multiple concurrent sessions (host tabs)**: a terminal-grid session strip sits above the remote pane — one tab per session labelled `user@host` (so two logins to the same host stay distinguishable), each with its own `\u00d7`, plus `[+]` to connect another. The story's premise held up: the engine already keyed everything by `SessionId`, and the single global `remotePane` singleton was the only thing making the app single-session. It became a **per-session registry** (`remotePaneFor(id)`), with the page's `remotePane` a `$derived` off the active session — so all 27 existing call sites kept working unchanged while switching tabs now swaps the entire pane state (path, listing, selection, sort, expanded tree, filter) rather than re-listing. **Two real defects surfaced, both invisible to the type checker.** (1) `loadRemote` read the derived pane _again after_ awaiting `listDir`, so switching tabs mid-listing would file one host's entries into another host's pane — precisely the cross-session corruption per-session panes exist to prevent. It (and `revealHit`) now resolve the pane from the session id **before** the await and write through that reference; `loadRemote` takes an explicit session argument so connect and tab-switch never depend on when the derived recomputes. (2) shell events were dispatched to a **single global handler pair**, so with several terminals mounted every shell's output went to whichever mounted last and only one tab's shell would have worked; `setShellHandlers` became `subscribeShell` over a subscriber set, returning an unsubscribe. Each session's terminal stays mounted with only the active one displayed (stacked absolutely, so a hidden xterm keeps its measured size), which is what makes a tab switch return to that host's live shell **with its scrollback**, rather than opening a fresh one. Disconnect closes one tab, not the world: it drops just that session's pane, health ring, and edit sessions, and only clears diff mode when no remote pane is left to compare against; a reconnecting session is marked `\u27f3` rather than removed, so a tab does not vanish while the supervisor retries. One **deliberate lint exception**, documented in place: the pane registry is a plain `Map`, not a `SvelteMap`, because `remotePaneFor` inserts on first use from inside a `$derived` and mutating reactive state there throws `state_unsafe_mutation` — verified with a throwaway test, not assumed, and nothing needs the map itself to be reactive. 18 new tests (5 per-session pane independence/identity/parking-pane/freeing, 9 tab strip incl. same-host disambiguation, closing only the clicked session, and the reconnecting marker, 4 shell-event fan-out incl. the last-mounted-wins regression). Verified live that the strip is correctly absent while disconnected and that the module now exports only the registry API; the multi-tab behaviour itself needs two real servers and a Tauri backend, so it rests on the tests.

- **E8-S7 — remote search**: `crates/engine/src/search.rs` prefers one server-side `find <root> -iname '*q*'` over E8-S1's exec channel and falls back to a bounded SFTP walk when the server refuses exec; Cmd/Ctrl+F opens a `find` prompt over the remote pane's directory and choosing a hit jumps the pane to its **containing** directory and selects it there, so the normal operations apply immediately. Which strategy ran is reported (`SearchOutcome::strategy`) and shown in the footer — a search that quietly took twenty times longer should be visible, not a mystery. **Both strategies return paths only, no metadata**, deliberately: `find` cannot portably report an entry's type (`-printf` is GNU-only, absent on BSD/macOS/busybox) and a `stat` per hit would undo the single-round-trip advantage that is the whole point; letting the walker return more would make results silently change shape with the strategy, which is worse than both being minimal. **A non-zero `find` exit with output is still a usable result** — `find` exits non-zero if _any_ subdirectory was unreadable, which searching `/` as a non-root user is the normal case, not an error; treating it as failure would send every such search down the slow walk, which would hit exactly the same unreadable directories. Only an empty non-zero result (127, refused exec) falls back. The user's query is **glob-escaped before shell-quoting** — `-iname` takes a glob, so an unescaped `a*b` would silently match far more than was typed; the test for this initially passed vacuously because the test server's `find` stub did substring matching, so the stub was given a real backtracking glob matcher and a decoy file, and the test was then confirmed to fail when the escaping is removed. **Nothing is silently truncated**: every bound (500 results, depth 12, 50 000 entries examined, the exec timeout) sets `truncated`, which the footer states. The walk never follows symlinks (project rule: escapes the root, and a cycle would run forever) and skips unlistable directories rather than failing the whole search. Cancellation crosses IPC **by id** — the command is awaited and offers no handle, so the frontend names the search up front and `cancel_search` reaches it through a registry in `AppState`, whose entry the search always removes before returning. The store's own test found a **real race**: `run()` awaited the previous search's cancellation _before_ claiming the slot, leaving a window in which two calls both saw "nothing running"; the state transition is now synchronous, and a superseded search's late result or late failure is discarded rather than clobbering the newer one. Searching is explicit (Enter), not as-you-type — the opposite trade-off from the `/` filter (E8-S13), which narrows already-loaded rows for free — and while a search runs Escape cancels it rather than closing, so an expensive walk started by mistake can be stopped without losing the query. Also hoisted the `shell_quote` helper, which had been **duplicated in `tarstream.rs` and `integrity.rs`**, into `exec.rs` before adding a third copy. 30 new tests (7 unit: glob escaping, command construction, injection, output parsing, cap reporting, CRLF, base names; 9 integration: exec path at three depths, walk fallback, the two strategies agreeing on the same tree, literal glob matching, depth and result bounds each reporting truncation, cancel mid-walk and mid-exec each leaving the session usable, invalid input rejected before any round-trip; 8 store, 13 component). Verified in the preview that Cmd+F correctly does nothing while disconnected; the dialog itself could not be driven live without a Tauri backend, so its behaviour rests on the component tests.

- **E8-S6 — pane diff mode**: a `[diff]` toggle in the topbar (only rendered while connected — it needs two trees) marks both panes' rows with terminal-grid glyphs computed in `src/lib/diff.ts`, a pure, unit-tested module. Rows are paired by path **relative to each pane's root**, rebuilt from the flattened tree's depth tags rather than by slicing absolute paths, so a Windows local pane (`C:\\src\\app\\main.rs`) still lines up with a POSIX remote one (`/srv/app/main.rs`). Only what is loaded participates (root listings plus expanded children), so expanding a folder deepens the comparison; no hashing, as scoped. **Deviation — four marks, not three.** The story specifies one "differs (size or mtime)" mark, but taken literally that mark fires on nearly every row, because **this app does not preserve mtime across a transfer**: the engine writes the destination and lets the filesystem stamp it, so a file uploaded a minute ago already has two different timestamps with identical bytes. A single `≠` would have meant "different, or you copied one to the other, who knows"; dropping mtime instead would have hidden same-size edits. So the cases are separated — `≠` sizes differ (contents certainly differ, tinted amber) and `~` same size, different mtime (_may_ differ, and also exactly what a completed copy looks like, so it stays quiet and is excluded from "transfer the differences", which would otherwise re-send the same bytes forever). `=` same, `+` only here. Comparison tolerates 2s of clock skew (the rsync convention: SFTP carries whole seconds, FAT stores even seconds, NTP drifts). **Deviation — `-` is not rendered.** Marking "only-remote" _in the local pane_ would need a phantom row for a file that pane does not contain, which the flattened tree has no representation for and which could not be selected or transferred; instead each pane marks its own extras `+`, so an entry present on one side only is visible as `+` there. **Deviation — "transfer the differences" acts on depth-0 rows only.** A nested row's destination is a directory _inside_ the other tree that may not exist there, so enqueueing it would fail at transfer time, after the user was told it was queued; root-level rows always have a destination that exists, and a root-level directory marked `+` carries its whole subtree anyway. Deeper differences stay marked; acting on them means navigating in, where they become root-level. Directories are compared by existence only — their size says nothing about their contents and their mtime moves whenever a child is touched. Diff mode is deliberately **not persisted** (unlike `[follow]`): it belongs to the particular pair of directories on screen, and it clears on disconnect. Colour follows the sparse-accent rule — `=` and `~` stay dim, only `+` takes the accent — and every glyph carries a `title`/`aria-label`, so the meaning is never colour-only. 27 new tests (relative-path reconstruction incl. Windows separators and depth backtracking, every mark, tolerance boundary, null mtime, kind mismatch, nested trees, same-name-different-depth, the transfer-set filters, palette gating, store toggle, and component rendering). Verified in the preview that the toggle and the column are correctly **absent** while disconnected and that the grid template switches to five columns; the marks themselves could not be seen live, since without a Tauri backend neither pane lists anything.

- **E8-S5 — shell → pane cwd sync**: `cd` in the [shell] tab and the remote pane follows. Shells already announce their cwd to the terminal after each prompt (that is how terminal tabs remember directories), so `src/lib/osc.ts` reads the same **OSC 7** (`file://host/path`) and **OSC 1337** (`CurrentDir=`) sequences straight out of the PTY byte stream — no server-side configuration, and **nothing is written to the user's shell**. A `[follow]` toggle sits beside the tabs (persisted, **opt-in/default off**, hidden while disconnected). The scanner is stateful by necessity: a PTY delivers arbitrary chunk boundaries, so sequences routinely straddle them — it retains incomplete tails (bounded at 4 KB so a stray introducer inside `cat`-ed binary output can't grow it) and keeps a streaming `TextDecoder` so split UTF-8 codepoints survive. Writing the byte-by-byte split test **found a real bug**: a chunk ending on a lone `ESC` was dropped, because the introducer search needs both `ESC` and `]` — fixed by retaining a trailing `ESC`. Following is skipped when the pane is already at that path, so the per-prompt announcements don't re-list continuously. **The reverse direction ("cd here" from a pane) was deliberately not built**, though the story lists it as optional: typing into the shell is destructive when it isn't at a prompt — `cd /x\n` sent while the user is in `vim` corrupts their buffer, and there is no reliable way to detect a prompt. It deserves its own story with an explicit UX answer rather than a footgun bolted on here. 16 scanner tests (both encodings, both terminators, percent-decoding, multiple-per-chunk, two-chunk and byte-by-byte splits, split UTF-8, unrelated OSC like title-setting, relative/malformed rejection, buffer bound, reset) plus a ui-store toggle test.
